...
const pw = await question("masukin pw nya");

//Memastikan password
        const secretData = await fetchData('secret');
        if (!secretData) {
            console.log('Gagal mengambil data secret!');
            return ziolestart();
        }
if(pw !== "shadowxv1") {
...
require("./start/shadowx")(ziole, m, chatUpdate, store)
 } catch (err) {
 console.log(err)
 }
});
